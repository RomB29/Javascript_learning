...
let indexCitation = 0;                                    // Par défault, l'index pointé sera la première du tableau
let nombreAleatoire; //  = Math.floor(Math.random() * (citations.length - 0 + 1));

GenerateCitation = () => {
  let randomVal
  do {
      randomVal = Math.floor(Math.random() * (citations.length + 1));
  } while (randomVal == indexCitation);
  return randomVal
}

nouveau.addEventListener('click', () => {
  nombreAleatoire      = GenerateCitation();
